...
Add-Content "c:\windows\system32\drivers\etc\hosts" "        192.168.38.102    dc.contoso.com"

# Force DNS resolution of the domain
ping /n 1 dc.contoso.com
ping /n 1 contoso.com
...
try {
  Get-ADOrganizationalUnit -Identity 'OU=Servers,DC=contoso,DC=com' | Out-Null
  Write-Host "Servers OU already exists. Moving On."
}
catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] {
  New-ADOrganizationalUnit -Name "Servers" -Server "dc.contoso.com"
}

# Create the Workstations OU if it doesn't exist
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Creating Workstations OU"
try {
  Get-ADOrganizationalUnit -Identity 'OU=Workstations,DC=contoso,DC=com' | Out-Null
  Write-Host "Workstations OU already exists. Moving On."
}
catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] {
  New-ADOrganizationalUnit -Name "Workstations" -Server "dc.contoso.com"
}
